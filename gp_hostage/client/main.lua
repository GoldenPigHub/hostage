...
AddEventHandler('gp_hostage:takeHostage', function()
	callTakeHostage()
end)
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				lib.notify({
					type = 'error',
					title = locale('title'),
					description = locale('nobody_near'),
					position = 'bottom'
				})
			end
		else
